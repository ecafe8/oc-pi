...
- Pi SDK 提供 `InteractiveMode 交互模式`，包含 editor 编辑器、chat history 聊天历史 与 built-in commands 内建命令
- Pi extension 扩展还提供 `ctx.ui.select`、`ctx.ui.confirm`、`ctx.ui.input`、`ctx.ui.editor`、`ctx.ui.notify` 等交互 API
- 这类能力适合直接作为 `oc-pi-cli` 的交互工作台基础

### RPC Runtime RPC 运行时

- Pi 提供 `pi --mode rpc` 与 SDK `runRpcMode(runtime)`
- 这类能力适合作为未来外部应用接入、消息驱动或远程控制的基础设施
...
### Product Workbench 信息架构

- Pi 提供 TUI 基础能力，但没有内建面向 `goal 目标`、`docs 文档`、`tasks 任务`、`review 审查` 的产品工作台信息架构
- `interactive-workbench 交互工作台` 仍属于 `oc-pi-cli` 需要自建的产品能力

### External Integration Workflow 外部集成工作流

- Pi 提供 RPC 基础，但没有内建企微、消息流转、外部审批或外部系统工作流接入
- 如果未来需要连接企微消息或其他应用，`oc-pi-cli` 仍需自建产品层流程与适配逻辑
...
| Interactive UI 交互界面 | InteractiveMode 交互模式、UI dialogs 对话框 | 产品级工作台视图与工作流 |
| RPC Integration RPC 集成 | rpc mode 模式、SDK 运行时 | 外部应用接入流程与协议 |
| MCP 集成 | 无内建 | 完整自建 |
