...
outputTarget: product-goal # 输出目标逻辑槽位
...
- `outputTarget 输出目标`: `product-goal 产品目标槽位` 与后续 `capability-map 能力地图槽位`

### Goal Reviewer 目标审查者

- `roleId 角色标识`: `goal-reviewer`
- `responsibility 职责`: 审查目标定义、能力拆解与范围边界
- `outputTarget 输出目标`: `review-notes 审查记录槽位`

### Doc Writer 文档编写者

- `roleId 角色标识`: `doc-writer`
- `responsibility 职责`: 生成或更新规划文档、功能文档与说明文档
- `outputTarget 输出目标`: `feature-plan 功能规划槽位`、`mvp-scope MVP 范围槽位`、`next-summary 下一步摘要槽位`
...
- `outputTarget 输出目标`: `implementation-code 实现代码槽位`
...
注意：下面的 `provider 提供商` 与 `model 模型` 只是产品规划示例值，不保证可直接作为某一 SDK 或 API 的最终可执行模型 ID，应在实现层映射为真实 provider/model 标识。
...
- `artifact-routing 产物路由` 依赖 `outputTarget 输出目标` 与逻辑槽位映射决定写入位置
- `interactive-workbench 交互工作台` 依赖角色配置决定界面里展示哪些可切换角色

## Model Identifier Note 模型标识说明

Model Identifier Note 模型标识说明用于避免把当前文档中的模型名误解为最终执行时的 provider model ID。

- 当前文档中的 `planner-model`、`reviewer-model` 等值属于逻辑占位标识
- 真正可执行的模型 ID 应在 provider 适配层中完成映射
- 这样做可以避免文档在产品规划阶段被具体平台模型命名锁死
