...
| `mvp-scope MVP 范围槽位` | `planning/` | `planning/mvp-features.md` |
| `next-summary 下一步摘要槽位` | `tasks/` | `tasks/next-up.md` |
| `review-notes 审查记录槽位` | `tasks/` | `tasks/review-notes.md`（待创建，第一版临时落点） |
...
- 第一版允许 `review-notes 审查记录槽位` 继续临时落在 `tasks/` 下，后续再独立目录
...
- Docs Writing Rule 文档编写规则（`protocols/docs-writing-rule`，待创建）
