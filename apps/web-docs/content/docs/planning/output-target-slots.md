...
- 作用: 承载规划闭环收束后的阶段性交接摘要
- kind 产物类型: `docs`

### handoff-next-up 下一步指引槽位

- 作用: 承载从交接摘要提炼出的动态下一步动作
- kind 产物类型: `docs`

### next-summary 下一步摘要槽位

- 作用: 历史兼容槽位；保留旧路径映射，但不再作为当前 Stage 4 的 source-of-truth 真源输出
- kind 产物类型: `status`
...
Role Binding Example 角色绑定示例用于说明角色配置如何引用逻辑槽位而不是直接写死路径。当前 `doc-writer 文档编写者` 的默认 `outputTarget 输出目标` 仍可指向主规划文档槽位，但具体 stage 阶段可通过 `primaryOutputSlot 主输出槽位` 与 `additionalOutputSlots 附加输出槽位` 覆盖到 Stage 4 的 handoff 双输出。
...
- 默认仍保持单角色到单槽位的稳定映射，但稳定阶段允许由 stage contract 阶段契约声明 `primaryOutputSlot 主输出槽位 + additionalOutputSlots 附加输出槽位`
- 不支持复杂优先级路由
- 先支持单角色到单槽位的稳定映射
- 路由结果先以单一默认路径映射为主
...
- `next-summary 下一步摘要槽位` 是否应长期保留为兼容槽位，还是在后续版本中彻底移除
