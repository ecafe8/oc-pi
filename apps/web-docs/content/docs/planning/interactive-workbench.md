...
- 用户必须能持续看到当前 `status 状态`，尤其是 `thinking 方案生成中`、`running 运行中`、`reviewing 审查中`、`blocked 已阻塞`
...
- 当前总状态，例如 `thinking 方案生成中`、`idle 空闲` 或 `running 运行中`
...
- 第一版的显式命令统一采用 `slash command 斜杠命令 + kebab-case` 单段格式，例如 `/docs-exec-confirm`、`/docs-status-show`，不采用 `/goal new` 这种带空格的层级命令
- 输入 `/` 后，命令候选应随前缀动态过滤，并支持 `Tab` 完成当前候选项
- 对支持参数补全的命令，补全完成后应保留继续输入参数或自然语言描述的光标位置
...
```text
+--------------------------------------------------------------------------------------------------+
| model: gpt-5-mini | ctx: 42k / 128k (33%) | version: 0.0.0 | mode: planning | status: idle |
+--------------------------------------------------------------------+-----------------------------+
| Chat 聊天区                                                        | Info 信息区                 |
|                                                                    |                             |
...
其中“规划后继续对话修正”是第一版必须优先做好的体验，因为 `goal-to-docs 目标到文档` 本质上需要人工讨论、修正与确认，而不是每次出方案都强制停在单独确认节点。
...
Docs Workflow Commands 文档工作流命令用于驱动 `goal-to-docs 目标到文档` 主流程。当前推荐路径已经收敛为“普通聊天负责目标设定、补充约束、方案修正；显式命令负责执行确认、执行取消与少量状态查询”。

### Primary Recommended Flow 主推荐流程

Primary Recommended Flow 主推荐流程用于固定当前推荐交互，不再要求用户记忆多个阶段性 docs 命令。

- 用户直接通过普通聊天输入目标、补充约束、提出修正意见
- 工作台将自然语言作为 docs 规划输入，并在消息流中以 `Planner 规划者` 与 `Reviewer 审查者` 两个 AI 角色流式展示规划与审查对话
- 当前规划阶段默认允许最多 `3` 轮 `planner-reviewer 编写者-审查者` 收敛循环
- 当工作台已经生成方案后，用户可以继续通过普通聊天调整；若确认可以落文档，则可直接说“可以落文档”或使用 `/docs-exec-confirm`
- 若当前待执行方案需要放弃，则使用 `/docs-exec-cancel` 取消

### Compatibility Docs Commands 兼容文档命令

Compatibility Docs Commands 兼容文档命令用于记录当前仍保留但已不再是推荐主路径的旧命令。

#### `/docs-goal-new`

Function 功能：开始新的 `docs goal 文档目标` 设定流程。当前它属于兼容命令，而不是推荐主入口。
...
Function 功能：基于当前 `goal 目标` 请求 AI 生成执行方案。当前它属于兼容命令，而不是推荐主入口。
...
- 成功后，工作台会更新方案摘要与步骤列表，并回到可继续对话修正的规划态。

Impact 影响：

- 会刷新右侧 `Plan 执行计划` 区的步骤列表与摘要。
- 会更新当前运行时状态到 `thinking 方案生成中`，并在方案生成后回到 `idle 空闲` 以便继续对话修正。
...
Function 功能：基于当前目标重新生成 docs 方案。当前它属于兼容命令，而不是推荐主入口。
...
- 不会直接执行，也不会直接写文件。
- 用户可继续通过普通聊天修正方案；确认执行时，可直接输入“可以落文档”等自然语言，或使用 `/docs-exec-confirm`。
...
Goal 目标：把已经在聊天中确认好的方案推进到实际运行阶段。
...
- 用户输入 `/session-resume ` 后，工作台应提供最近 session 会话列表补全，候选至少显示 `session name 会话名` 或 `session id 会话标识`，并附带更新时间或 goal 摘要。
...
- 创建后自动切换到新的 fork 会话。
- 用户输入 `/session-fork ` 后，工作台应允许通过最近 session 会话候选完成 fork 来源选择；不输入参数时默认从当前会话 fork。
...
Slash Command Rule 斜杠命令规则用于固定第一版工作台的命令输入语法，避免后续实现阶段同时出现 `/goal new`、旧 `/docs-goal-new` 兼容入口、自然语言伪命令三套不一致交互。

- 所有显式命令必须以 `/` 开头
- 所有显式命令必须使用 `kebab-case 连字符命名`
- 第一版不采用带空格的层级命令，例如不使用 `/goal new`
- 第一版优先支持确定性动作命令，不强制把规划过程拆成多条显式 docs 命令
...
Goal Input Behavior 目标输入行为用于说明当前实现中普通聊天与兼容 `/docs-goal-new` 命令的配合方式，避免文档把它误写成独立模式切换。

- 当前推荐方式是直接发送自然语言目标，系统会把该消息作为 docs 规划输入并生成方案
- 用户输入 `/docs-goal-new` 后，系统只追加一条提示消息，提醒用户继续描述新的目标
- 随后的自然语言正文仍通过统一聊天入口提交，而不是进入单独的输入模式
- 当前实现下，自然语言输入会同步更新当前 `goal 目标`，并写入当前会话状态
- 当前实现也允许使用 `/docs-goal-new <自然语言目标描述>` 直接在单次命令中提交目标文本
- `/docs-plan-run <补充约束>` 与 `/docs-plan-retry <修正规则>` 可在命令后追加自然语言描述，作为本次规划的补充上下文
- 第一版允许后续再评估是否支持更严格的 goal capture 目标采集模式，但这不是当前真源行为

## Streaming Feedback 流式反馈

Streaming Feedback 流式反馈用于说明为什么工作台在长时间 `goal-to-docs 目标到文档` 执行中不能只显示最终结果。

- 主聊天时间线应持续追加高层阶段状态，例如“正在思考产品愿景”“正在编写产品能力大纲文档”“正在审查功能规划文档”。
- 在聊天规划阶段，主聊天时间线也应允许用户看到 `Planner 规划者` 与 `Reviewer 审查者` 的流式对话，而不是只在最后展示单个结果。
- 这些阶段状态用于帮助用户理解当前相位是 `thinking 思考`、`writing 编写` 还是 `reviewing 审查`，而不是暴露底层模型全部原始推理。
- 工作台应提供一个限高的 `live draft 实时草稿` 预览区域，显示当前 writer 正在生成的文档片段。
- `live draft 实时草稿` 区必须独立滚动或截断，不能无限推高主聊天历史区域。
- `live draft 实时草稿` 仅表示生成中预览，不承诺与最终落盘内容逐字一致。
...
- 注册 `session-new`、`session-list`、`session-resume`、`session-fork`、`docs-exec-confirm`、`docs-exec-cancel`、`docs-review-latest`、`docs-status-show`、`workbench-help-show`、`workbench-thinking-toggle`、`workbench-pane-chat-focus`、`workbench-pane-thinking-focus`、`workbench-pane-info-focus`
- 当前代码仍兼容 `docs-goal-new`、`docs-plan-run`、`docs-plan-retry`，但它们不再是推荐主流程入口
- 负责命令参数解析与命令结果结构化
- 不直接负责最终 UI 渲染
...
workbenchState:
  session:
    ...
    currentStageStatus: pending # 当前阶段状态
