...
- `internal session state files 内部会话状态文件` 指工作台为了恢复会话而写入的内部状态文件，这类写入不属于用户目标产物。
...
Goal 目标：告诉工作台“接下来我要重新定义当前文档目标”，让用户随后用自然语言输入新的目标正文。
...
- 后续自然语言输入通常会被用户视为新的目标正文，并成为后续 `/docs-plan-run` 的输入基础。
...
- 注册 `docs-goal-new`、`docs-plan-run`、`docs-plan-retry`、`docs-exec-confirm`、`docs-exec-cancel`、`docs-review-latest`、`docs-status-show`、`workbench-help-show`、`workbench-thinking-toggle`、`workbench-pane-chat-focus`、`workbench-pane-thinking-focus`、`workbench-pane-info-focus`
- 负责命令参数解析与命令结果结构化
- 不直接负责最终 UI 渲染
