...
- 第一版的显式命令统一采用 `slash command 斜杠命令 + kebab-case` 单段格式，例如 `/goal-new`、`/status-show`，不采用 `/goal new` 这种带空格的层级命令
...
- `/goal-new` 用于进入新的 `goal 目标` 输入模式，输入或替换当前目标
- `/goal-run` 用于基于当前目标请求 AI 给出执行方案或进入执行前准备
- `/goal-retry` 用于重试当前主流程
- `/confirm-execute` 用于接受当前 AI 方案并开始执行
- `/cancel-run` 用于在运行中停止当前动作
...
Slash Command Rule 斜杠命令规则用于固定第一版工作台的命令输入语法，避免后续实现阶段同时出现 `/goal new`、`/goal-new`、自然语言伪命令三套不一致交互。
...
Goal Input Mode 目标输入模式用于说明为什么 `/goal-new` 不应被第一版实现为“命令 + 空格 + 大段正文”的单行解析。

- 用户输入 `/goal-new` 后，系统进入 `goal input mode 目标输入模式`
- 随后的编辑器正文用于承载新的目标文本
- 用户提交正文后，工作台再把这段文本作为新的 `goal 目标` 写入当前会话状态
- 第一版允许后续再评估是否支持 `/goal-new <inline text>`，但这不是当前真源要求
