...
- `/docs-goal-new` 用于进入新的 `docs goal 目标` 输入模式，输入或替换当前目标
- `/docs-plan-run` 用于基于当前目标请求 AI 给出执行方案或进入执行前准备
- `/docs-plan-retry` 用于重试当前 docs 主流程
- `/docs-exec-confirm` 用于接受当前 AI 方案并开始执行
- `/docs-exec-cancel` 用于在运行中停止当前 docs 动作
- `/docs-status-show` 用于查看当前 docs 会话状态摘要
- `/docs-review-latest` 用于查看最近一次 docs 审查结论
- `/workbench-help-show` 用于查看工作台帮助
- `/workbench-thinking-toggle` 用于折叠或展开 Thinking 区
- `/workbench-pane-chat-focus` 用于将滚动焦点切到聊天区
- `/workbench-pane-thinking-focus` 用于将滚动焦点切到 Thinking 区
- `/workbench-pane-info-focus` 用于将滚动焦点切到右侧信息区
...
### Goal Input Mode 目标输入模式

Goal Input Mode 目标输入模式用于说明为什么 `/docs-goal-new` 不应被第一版实现为“命令 + 空格 + 大段正文”的单行解析。

- 用户输入 `/docs-goal-new` 后，系统进入 `goal input mode 目标输入模式`
- 随后的编辑器正文用于承载新的目标文本
- 用户提交正文后，工作台再把这段文本作为新的 `goal 目标` 写入当前会话状态
- 第一版允许后续再评估是否支持 `/docs-goal-new <inline text>`，但这不是当前真源要求
