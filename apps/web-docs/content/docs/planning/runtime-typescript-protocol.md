...
- `core.ts`: `WorkMode 工作模式`、`RoleId 角色标识`、`SlotId 槽位标识`、`StageId 阶段标识`、`StageStatus 阶段状态`、`ReviewStatus 审查状态`、`RuntimeStatus 运行时状态`、`WriteMode 写入模式`、`CommandId 命令标识`
- `artifacts.ts`: `SlotDefinition 槽位定义`、`ResolvedSlotTarget 已解析槽位目标`
- `review.ts`: `ReviewFinding 审查问题`、`ReviewResult 审查结果`
- `roles/types.ts`: `RoleConfig 角色配置`
- `planning/goal-to-docs/types.ts`: `GoalToDocsStageContract 目标到文档阶段契约`、`GoalToDocsStageRecord 目标到文档阶段记录`、`GoalToDocsRunRecord 目标到文档执行记录`
- `workbench/types.ts`: `TimelineItem 时间线条目`、`WorkbenchState 工作台状态` 及其子状态
- `provider-adapters/types.ts`: `ProviderModelMapping 提供商模型映射`
...
- `RoleConfig 角色配置` 中的 `model 模型` 表示逻辑模型标识，而不是 provider SDK 的最终 `resolvedModelId 已解析模型标识`
- `ProviderModelMapping 提供商模型映射` 负责把 `RoleConfig.model` 中的逻辑模型标识映射为真实执行 ID
...
- `GoalToDocsStageContract.completionRule 完成规则` 在第一版固定为 `'accepted'`，这是有意约束，不是冗余占位字段
- `TimelineItem.createdAt 创建时间` 与其他时间字段第一版统一使用 `ISO 8601` 字符串
...
- [Review Loop 审查循环草案](./review-loop)
